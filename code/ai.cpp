...
const int MAX_DEPTH = 500000;          // Maximum depth for pruning
int MAX_NODES = 15000;      // Maximum number of nodes to evaluate
...
int countTotalPieces(GameModel& model) {
    int count = 0;
...
int evaluateBoard(GameModel& model, int depth)
{
    ...
    // Total value combining all factors
        // Positional advantage (weighted positions)
    int positionalScore = 0;
    for (int y = 0; y < BOARD_SIZE; y++) {
        for (int x = 0; x < BOARD_SIZE; x++) {
            Square s = { x, y };
            Piece piece = getBoardPiece(model, s);
    ...
    return pieceDiff + cornerValue + edgeValue + (mobility - opponentMobility) + positionalScore;
}
...
int minimax(GameModel& model, int depth, int alpha, int beta, bool maximizingPlayer)
{
    ...
    // Ordenar movimientos por número de piezas que se pueden voltear (mayor primero)
    std::sort(validMoves.begin(), validMoves.end(), [&](const Square& a, const Square& b) {
        return countTotalFlippedPieces(model, a) > countTotalFlippedPieces(model, b);
        });
...
Square getBestMove(GameModel& model)
{
    ...
    int pieces = countTotalPieces(model);
    if (pieces < 20)
        MAX_NODES = 20000; // más profundidad en apertura
    else if (pieces > 50)
        MAX_NODES = 50000; // más profundidad en final
    else
        MAX_NODES = 15000; // medio juego
